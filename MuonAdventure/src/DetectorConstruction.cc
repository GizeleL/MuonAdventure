...
void DetectorConstruction::DefineMaterials() {
    ...
    // Water
    G4Material *fWater = new G4Material("Water", density = 1. * g / cm3, 2);
...
G4VPhysicalVolume *DetectorConstruction::ConstructVolumes() {
    ...
    G4double scint_radius = fBoxSize / 2.;   // Volume radius "fScint_box"
    G4double scint_half_z = 1.8787 / 2. * m; // Volume height "fScint_box"

    G4Tubs *sBox = new G4Tubs("Container", 0., scint_radius, scint_half_z, 0., 360. * deg);
    // G4Box *
    ...
    G4Color transparentColor_scint(0., 0., 1.0, 0.5);
    G4Color transparentColor_housing(0.8, 0.8, 0.8, 0.5);

    G4VisAttributes *scint_va = new G4VisAttributes(transparentColor_scint);
    scint_va->SetForceSolid(true);
    fLBox->SetVisAttributes(scint_va);
